// app.js — client-side prototype with optional Firebase integration
...
function addProduct(){
  ...
  if(firebaseEnabled && fbDB){
    (async ()=>{
      try{ await fbDB.collection('products').add({name,qty,updatedBy:session.email,updatedAt:Date.now()}); await logActivity(session.email,`เพิ่มสินค้า ${name}`); renderProducts(); }catch(e){console.error('addProduct fb',e); alert('เพิ่มสินค้าไม่สำเร็จ');}
    })();
    return;
  }
...
function deleteProduct(id){
  ...
  if(firebaseEnabled && fbDB){
    (async ()=>{
      try{ const doc = await fbDB.collection('products').doc(id).get(); const p = doc.exists? doc.data() : {name:id}; await fbDB.collection('products').doc(id).delete(); const s = load('session'); if(s) await logActivity(s.email,`ลบสินค้า ${p.name}`); renderProducts(); }catch(e){console.error('deleteProduct fb',e); alert('ลบไม่สำเร็จ')}
    })();
    return;
  }
